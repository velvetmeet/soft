...
// Включить/выключить cloaking
const CLOAKING_ENABLED = false;

// Включить/выключить логирование ботов в Cloudflare logs
const BOT_LOGGING_ENABLED = true;

// ========== СПИСОК БОТОВ ==========
// User-Agent патэрны которые точно являются ботами
// Обновляется вручную. Источник: github.com/monperrus/crawler-user-agents
const BOT_PATTERNS = [
  // Meta crawlers
  "facebookexternalhit",
  "facebookcatalog",
...
// IP-диапазоны Meta (актуально на 2026)
// Источник: developers.facebook.com/docs/sharing/webmasters/crawler/
...
<title>Mindful Mornings — Slow Living Journal</title>
<style>
...
<p>A weekly note about rituals, rain, books, and quiet rooms — for people who want a slower life after fifty.</p>
...
<p>We talk a lot about productivity. We talk less about <em>permission</em> — permission to slow down, to read for an hour, to call an old friend without a reason.</p>
...
<div class="footer">© 2026 Mindful Mornings · A reading and rituals journal</div>
</body>
</html>`;
...
function logHuman(request) {
  if (!BOT_LOGGING_ENABLED) return;
  // Sample 5% of human traffic для baseline
...
export async function onRequest(context) {
  ...
  // CLOAKING — отдаём заглушку ботам
  ...
  // NORMAL FLOW — обычный юзер или бот без cloaking
